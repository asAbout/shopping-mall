<template>
  <header class="header">
    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          ...
          <a href="javascript:void(0)" class="navbar-link" v-show="user">{{nickname}}</a>
          <a href="javascript:void(0)" class="navbar-link" @click="toLogin" v-show="logining">Login</a>
          <a href="javascript:void(0)" class="navbar-link" v-show="logout" @click="islogout">Logout</a>
          <div class="navbar-cart-container">
            <span class="navbar-cart-count" v-if="cartCount>0">{{cartClount}}</span>
    ...
    <div class="md-overlay" v-if="mask"></div>
  </header>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getgoodsCount() {
        axios.get('/users/goodsCount').then((response) => {
          let res = response.data
          if (res.status == '0') {
            this.$store.commit("initCartCount", res.result.goodsNum)
          }
        })
      }
    }
